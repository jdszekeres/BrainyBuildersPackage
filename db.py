...
cur = xconn.cursor()
cur.execute(
    "create table if not exists packages (id SERIAL, date text NOT NULL, locations jsonb NOT NULL, finaldestination text not null);"
)
xconn.commit()
del xconn
...
def add_package(location, finallocation, id=None):
    ...
    if not len(__selectall()) == 0:
        if id is None and select_package(__selectall()[-1][0]) is not None:
            print("first")
            cur.execute(
                "INSERT INTO packages (date, locations, finaldestination) VALUES ('{date}','{location}','{finaldestination}') RETURNING id;".format(
                    date=datetime.now().strftime("%A %B %d %Y %r"),
                    location=location,
            ...
            cur.execute(
                "INSERT INTO packages VALUES ('{id}','{date}','{location}','{finaldestination}') RETURNING id;".format(
                    date=datetime.now().strftime("%A %B %d %Y %r"),
                    location=location,
                    finaldestination=finallocation,
                    id=id,
...
def update_packages(id, location):
    ...
    package.append(
        {"date": datetime.now().strftime("%A %B %d %Y %r"), "location": location}
    )
    x = "UPDATE packages set locations = '{packages}' where id='{id}'".format(
        packages=str(package).replace("'", '"'), id=id
    )
    print(x)
    cur.execute(x)
    conn.commit()
...
def __package_say_goodlbye(id, locations, finaldestination):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE packages set finaldestination='{loc}' where id='{id}'".format(
            loc=finaldestination, id=id
        )
    )
    conn.commit()
